...
bool CIOCPImpl::AddTimerEvent(unsigned int interval, int event_flag, CMemSharePtr<CEventHandler>& event) {
	_timer.AddTimer(interval, event_flag, event);
	return true;
}

bool CIOCPImpl::AddSendEvent(CMemSharePtr<CEventHandler>& event) {
...
bool CIOCPImpl::AddRecvEvent(CMemSharePtr<CEventHandler>& event) {
	auto socket_ptr = event->_client_socket.Lock();
	if (socket_ptr) {
		if (!socket_ptr->IsInActions()) {
			if (CreateIoCompletionPort((HANDLE)(socket_ptr->GetSocket()), _iocp_handler, 0, 0) == NULL) {
...
bool CIOCPImpl::AddAcceptEvent(CMemSharePtr<CAcceptEventHandler>& event) {
...
bool CIOCPImpl::AddConnection(CMemSharePtr<CEventHandler>& event, const std::string& ip, short port) {
	auto socket_ptr = event->_client_socket.Lock();
	if (socket_ptr) {
		if (!socket_ptr->IsInActions()) {
			if (CreateIoCompletionPort((HANDLE)(socket_ptr->GetSocket()), _iocp_handler, 0, 0) == NULL) {
...
bool CIOCPImpl::AddDisconnection(CMemSharePtr<CEventHandler>& event) {
	auto socket_ptr = event->_client_socket.Lock();
	if (socket_ptr) {
		if (!socket_ptr->IsInActions()) {
			if (CreateIoCompletionPort((HANDLE)(socket_ptr->GetSocket()), _iocp_handler, 0, 0) == NULL) {
...
bool CIOCPImpl::DelEvent(CMemSharePtr<CEventHandler>& event) {
...
bool CIOCPImpl::_PostRecv(CMemSharePtr<CEventHandler>& event) {
...
bool CIOCPImpl::_PostAccept(CMemSharePtr<CAcceptEventHandler>& event) {
	if (!__AcceptEx) {
...
bool CIOCPImpl::_PostSend(CMemSharePtr<CEventHandler>& event) {
	EventOverlapped* context = (EventOverlapped*)event->_data;
...
bool CIOCPImpl::_PostConnection(CMemSharePtr<CEventHandler>& event, const std::string& ip, short port) {
	EventOverlapped* context = (EventOverlapped*)event->_data;
...
bool CIOCPImpl::_PostDisconnection(CMemSharePtr<CEventHandler>& event) {
	EventOverlapped* context = (EventOverlapped*)event->_data;
...
void CIOCPImpl::_DoTimeoutEvent(std::vector<TimerEvent>& timer_vec) {
...
void CIOCPImpl::_DoEvent(EventOverlapped *socket_context, int bytes) {
