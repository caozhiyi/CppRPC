...
bool CParsePackage::PackageFuncRet(char* buf, int& len, int code, const std::string& func_name, const std::map<std::string, std::string>& func_str_map, std::vector<CAny>& ret) {
	...
	for (int i = 0; i < (int)func_str.size(); i++) {
		...
		case 'i':
			...
			if (!_SafeSprintf(false, cur, end, "i:%d|", any_cast<int>(ret[i - v_offset])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'c':
			...
			if (!_SafeSprintf(false, cur, end, "c:%c|", any_cast<char>(ret[i - v_offset])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 's':
			...
			if (!_SafeSprintf(false, cur, end, "s:%s|", (any_cast<std::string>(ret[i - v_offset])).c_str()))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'd':
			...
			if (!_SafeSprintf(false, cur, end, "d:%f|", any_cast<double>(ret[i - v_offset])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'l':
			...
			if (!_SafeSprintf(false, cur, end, "l:%ld|", any_cast<long>(ret[i - v_offset])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'b':
			...
			if (!_SafeSprintf(false, cur, end, "b:%d|", any_cast<bool>(ret[i - v_offset]) ? 1 : 0))
				return false;
			cur += strlen(cur);
			num--;
			break;
...
bool CParsePackage::PackageFuncCall(char* buf, int& len, std::string& func_name, const std::map<std::string, std::string>& func_str_map, std::vector<CAny>& param) {
	...
	for (int i = pos + 1; i < (int)func_str.size(); i++) {
		...
		case 'i':
			...
			if (!_SafeSprintf(false, cur, end, "i:%d|", any_cast<int>(param[i - (pos + 1 + v_offset)])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'c':
			...
			if (!_SafeSprintf(false, cur, end, "c:%c|", any_cast<char>(param[i - (pos + 1 + v_offset)])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 's':
			...
			if (!_SafeSprintf(false, cur, end, "s:%s|", (any_cast<std::string>(param[i - (pos + 1 + v_offset)])).c_str()))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'd':
			...
			if (!_SafeSprintf(false, cur, end, "d:%f|", any_cast<double>(param[i - (pos + 1 + v_offset)])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'l':
			...
			if (!_SafeSprintf(false, cur, end, "l:%ld|", any_cast<long>(param[i - (pos + 1 + v_offset)])))
				return false;
			cur += strlen(cur);
			num--;
			break;
		case 'b':
			...
			if (!_SafeSprintf(false, cur, end, "b:%d|", any_cast<bool>(param[i - (pos + 1 + v_offset)]) ? 1 : 0))
				return false;
			cur += strlen(cur);
			num--;
			break;
...
bool CParsePackage::_ParseParam(char* buf, char type, std::vector<CAny>& res) {
	char* cur = buf;
	if (*cur != type) {
		return false;
	}
	...
	{
	case 'i':
		res.push_back(CAny(atoi(cur + 2)));
		break;
	case 'c':
		res.push_back(CAny(cur + 2));
		break;
	case 's':
		res.push_back(CAny(std::string(cur + 2)));
		break;
	case 'd':
		res.push_back(CAny(atoll(cur + 2)));
		break;
	case 'l':
		res.push_back(CAny(atol(cur + 2)));
		break;
	case 'b':
		res.push_back(CAny(strcmp(cur + 2, "1") == 0?true : false));
		break;
	default:
		return false;
	}
...
bool CParsePackage::_ParseVec(char* buf, char type, std::vector<CAny>& res) {
	char* next = buf;
	char* pos = nullptr;
	char* cur = nullptr;
...
bool CParsePackage::_PackageVec(char* buf, char* end, char type, int index, std::vector<CAny>& vec) {
	char* cur = buf;
	...
	case 'i':
	{
		...
		std::vector<int> i_vec = any_cast<std::vector<int>>(vec[index]);
	...
	case 'c':
	{
		...
		std::vector<char> c_vec = any_cast<std::vector<char>>(vec[index]);
	...
	case 's':
	{
		...
		std::vector<std::string> s_vec = any_cast<std::vector<std::string>>(vec[index]);
	...
	case 'd':
	{
		...
		std::vector<double> d_vec = any_cast<std::vector<double>>(vec[index]);
	...
	case 'l':
	{
		...
		std::vector<long> l_vec = any_cast<std::vector<long>>(vec[index]);
	...
	case 'b':
	{
		...
		std::vector<bool> b_vec = any_cast<std::vector<bool>>(vec[index]);
